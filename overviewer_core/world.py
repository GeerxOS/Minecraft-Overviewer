...
class World(object):
        ...
        for root, dirs, files in os.walk(self.worlddir, followlinks=True):
            ...
            if mcas:
                ...
                if os.path.basename(rel) != "poi":
                    ...
                    if root == os.path.join(self.worlddir, "region") or \
                       root == os.path.join(self.worlddir, "dimensions", "minecraft", "overworld", "region"):
    ...
    def find_true_spawn(self):
        ...
        elif 'spawn' in data and 'pos' in data['spawn']:
            pos = data['spawn']['pos']
            disp_spawnX = spawnX = int(pos[0])
            spawnY = int(pos[1])
            disp_spawnZ = spawnZ = int(pos[2])
        else:
    ...
    def get_chunk(self, x, z):
        ...
        * For each chunk section:
        ...
        if chunk_data.get("Status", "") not in ("full", "postprocessed", "fullchunk",
                                                "mobs_spawned", "spawn", "",
                                                "minecraft:full", "minecraft:postprocessed",
                                                "minecraft:fullchunk", "minecraft:mobs_spawned",
                                                "minecraft:spawn"):
            raise ChunkDoesntExist("Chunk %s,%s doesn't exist" % (x,z))
